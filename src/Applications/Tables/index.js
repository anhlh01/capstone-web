...
// Applications.js
import { fetchData } from '../../Services/application';
    ...
    useEffect(() => {
        const fetchDataFromAPI = async () => {
            try {
                ...
                const responseData = await fetchData('applications', authToken, {
                    sortType: 'asc',
                    ...
                    <tbody>
                        {data.map(result => (
                            <tr key={result.id}>
                                ...
                                <td>{result.userId}</td>
                                <td>{result.approvedById}</td>
                                <td>{formatDateUtils(result.createdAt)}</td>
                                <td>{formatDateUtils(result.updatedAt)}</td>
                                <td>
                                    <Link to={`/applications/details/${result.id}`} title="See details">
